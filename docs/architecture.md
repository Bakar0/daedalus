...
Workspace content follows the contract in [workspace-content.md](workspace-content.md). Root `BRIEF.md` and `JOURNAL.md` files are visible, portable context while `.daedalus/` remains internal. Remote repositories have one global bare clone under `<DAEDALUS_HOME>/repos`; a successful fetch of the remote default branch produces a detached planning checkout in each workspace, which every later fetch moves forward. Agent sessions start in immutable task/session identity paths and create linked worktrees from the newest base-branch commit only when they need to modify a repository.
