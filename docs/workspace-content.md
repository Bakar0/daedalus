...
This bridge can be disabled globally in Settings. Disabling it removes only
exact, untouched files generated by Daedalus. User-authored or edited files are
preserved. When enabled, Daedalus also leaves pre-existing user files alone and
does not create or modify provider files inside attached repositories.
