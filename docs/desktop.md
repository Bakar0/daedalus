...
The desktop is a thin adapter over the same `ApplicationContext` used by `daedal`. Workspace selection stays in the left sidebar and a centered app-header control switches the complete workspace composition. Board mode, the first tab and the default, gives most width to the task canvas and keeps a narrower persistent inspector column: the task brief on top and the workspace's repositories under it (#27). Sessions mode replaces both with a compact vertical session navigator and a terminal-dominant work area. Workspace mode uses a compact, lazily expanded filesystem explorer beside a CodeMirror editor with Markdown preview. Root `BRIEF.md` and `JOURNAL.md` open like normal workspace files; freshly fetched repository references under `repos/` and session-owned worktrees appear in the explorer as folders. The repository list itself, with its add button, is on the board; the add button opens a fuzzy finder over the global repository library and also supports cloning a remote. There is no separate Activity view or permanent session rail. Session cards list task-linked and workspace-level sessions and show their durable lifecycle timestamps. Task briefs render as GitHub-flavored Markdown and switch to their portable plain-text source for edits.
...
Under the brief, behind a draggable border, sit the workspace's repositories (#27). Each row is a repository with its base branch and fetch state, and fetch, pull, terminal and dismiss actions; under it, one row per session worktree cut from it, with terminal, push and remove. The **+** on the heading, and the invite shown when nothing is attached yet, open the repository picker. The section moved here from the Workspace explorer so the board could be the first tab again: the reason Workspace had been first was that an empty workspace had nowhere else to add a repository. The section's height is remembered, dragged on the border above it, and the brief keeps a 140px floor. The board loads the workspace content for this and refreshes it on every data change, the same as the explorer does.
