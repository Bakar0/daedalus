...
What changed is the silence. With sessions or integrated terminals still live, Cmd+Q now shows a dialog naming exactly what would keep running and offering **Quit and keep N running** (the default), **Quit and archive all**, and **Cancel**, with a **Don't ask again** checkbox. Quitting with nothing live shows no dialog. Sessions with no resumable conversation are called out, because archiving stops those rather than preserving them.
...
The **On quit** setting holds the remembered answer — _Ask what to do_, _Keep sessions running_, or _Archive every session_ — and defaults to asking.

The quit item carries an action rather than `{ role: "quit" }`, because that role is a native macOS selector and Cmd+Q would never reach our code. Three paths remain outside this: quitting from the Dock, a logout or system shutdown, and `SIGINT`/`SIGTERM`. macOS routes the first straight to `NSApplication` and Electrobun 1.18.1 offers no hook into it; the others arrive with a deadline measured in seconds, which is no time to archive a board of sessions. All three fall through to what quitting has always done — everything keeps running — and never to a silent archive. So does a window that fails to acknowledge the dialog within two seconds, which keeps Cmd+Q from becoming a key that does nothing.
