...
The desktop is a thin adapter over the same `ApplicationContext` used by `daedal`. Workspace selection stays in the left sidebar and a centered app-header control switches the complete workspace composition. Board mode, the first tab and the default, gives most width to the task canvas and keeps a narrower persistent brief inspector. The workspace's repositories are chips beside the workspace name in the main header, which every mode shares (#27). Sessions mode replaces both with a compact vertical session navigator and a terminal-dominant work area. Workspace mode uses a compact, lazily expanded filesystem explorer beside a CodeMirror editor with Markdown preview. Root `BRIEF.md` and `JOURNAL.md` open like normal workspace files; freshly fetched repository references under `repos/` and session-owned worktrees appear in the explorer as folders. The repository list itself is the row of chips in the header, with its add button; the add button opens a fuzzy finder over the global repository library and also supports cloning a remote. There is no separate Activity view or permanent session rail. Session cards list task-linked and workspace-level sessions and show their durable lifecycle timestamps. Task briefs render as GitHub-flavored Markdown and switch to their portable plain-text source for edits.
...
Repositories belong to the workspace, not to a task, so they sit in the main header beside the workspace name, which every mode shares (#27). Each chip reads name, base branch and state (`daedalus main · Clean`, `hive main · ↓3 behind`, a spinner while a clone is arriving, the error when one failed). Clicking a chip opens a popover with the row the Workspace explorer used to draw: fetch, pull, terminal and dismiss on the repository, and one row per session worktree cut from it with terminal, push and remove. The **+** at the end of the row, and the "No repositories yet · Add a repository" chip shown when nothing is attached, open the repository picker. The chips moved here from the Workspace explorer so the board could be the first tab again: the reason Workspace had been first was that an empty workspace had nowhere else to add a repository. Outside the Workspace tab the app loads the workspace content for the chips and refreshes it on every data change, the same as the explorer does. A first version put the list under the task brief in the inspector; it read as the task's, which is why it moved.
