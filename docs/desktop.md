...
Two combinations fit none of those rows: an `in_progress` task with nothing ahead of base and nothing working, and a `todo` task whose agent is alive but idle. Both go to Running, below the tasks that are really working, because somebody started them and "is it still making progress?" is the question Running answers. A `todo` task in Running offers **Set in progress**. An `in_progress` task in Running with no live agent, because its agents exited or were archived or because the status was set by hand, says "No agent running" with the time its last agent stopped, and offers **Start**, **Mark done** and **Park** (sets it `blocked`); the drawer offers the same three. Archived sessions never put a task in Needs me or Running, but their worktrees still count as the task's output.
...
The card above the workspaces in the left column shows every active workspace's board at once (#35). The snapshot already carried every workspace's tasks, sessions and worktrees, so the scope only stops the renderer filtering them; nothing changes in the core or the RPC. The card sums the roll-up the workspace cards each carry, so its badge is the number of sessions waiting on you anywhere.

The lanes are the same lanes, computed by the same `laneFor`, over every workspace's tasks. Task numbers are per workspace, so each card names its workspace in front of its number, `atlas#24`, the way the CLI writes a reference; the `aria-label` and the Start next tooltip carry it too. Start, Start next, Second opinion, Draft brief and the ▾ dialog all act in the task's own workspace with that workspace's default provider and model, because the core refuses a task spawned anywhere else. Clicking a card opens the drawer as usual; a journal link in its timeline selects the task's workspace on the way to the Workspace view, since the journal is one workspace's file.

What belongs to one workspace is not there: the capture line, the create button, the board settings and the repositories column, so the lanes take the width. A task is captured on the board of the workspace it belongs to. The Workspace mode is disabled while every workspace is showing. Collapsed lanes and the last view are remembered for the scope as they are for a workspace.

The Sessions view keeps the scope. It lists every active workspace's sessions, each workspace's together in that workspace's own order, because a position is an order within one workspace and there is no list on the server for a drag across them to write; so the list is read-only there. Each card names its workspace before its task. **New** is disabled, because a session opens in one workspace: start a task from the board, or pick the workspace. Opening a session switches the terminal as usual, and the terminal's location line names the session's workspace. Archived workspaces stay out of the scope, as their cards stay out of the column.
